...
impl WrapSizerFrame {
    ...
    fn on_create(&self) {
        if let (Some(frame), Some(panel)) = (self.base.get(), self.m_panel.get()) {
            // Root sizer, vertical
            ...
            for n_check in 0..6 {
                let chk = wx::CheckBox::builder(Some(&panel))
                    ...
                    .build();
                sizer_mid_wrap.add_window_sizerflags(
                    Some(&chk),
            ...
            sizer_mid
                .add_sizer_sizerflags(Some(&sizer_mid_wrap), wx::SizerFlags::new(100).expand());
            sizer_root.add_sizer_sizerflags(
                Some(&sizer_mid),
    ...
    fn make_tool_bar<P: PanelMethods>(&self, panel: &P) -> wx::ToolBar {
