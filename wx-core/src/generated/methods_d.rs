...
pub trait DocManagerMethods: EvtHandlerMethods {
    ...
    fn activate_view(&self, doc: *mut c_void, activate: bool) {
        unsafe { ffi::wxDocManager_ActivateView(self.as_ptr(), doc, activate) }
    }
    ...
    fn create_view<D: DocumentMethods>(&self, doc: Option<&D>, flags: c_long) -> *mut c_void {
        unsafe {
            let doc = match doc {
                Some(r) => r.as_ptr(),
                None => ptr::null_mut(),
            };
            ffi::wxDocManager_CreateView(self.as_ptr(), doc, flags)
        }
    }
    ...
    fn get_any_usable_view(&self) -> *mut c_void {
        unsafe { ffi::wxDocManager_GetAnyUsableView(self.as_ptr()) }
    }
    ...
    fn get_current_view(&self) -> *mut c_void {
        unsafe { ffi::wxDocManager_GetCurrentView(self.as_ptr()) }
    }
    // NOT_SUPPORTED: fn GetDocumentsVector()
    // NOT_SUPPORTED: fn GetTemplatesVector()
    // BLOCKED: fn GetDocuments()
    fn get_file_history(&self) -> *mut c_void {
        unsafe { ffi::wxDocManager_GetFileHistory(self.as_ptr()) }
    }
    ...
    fn on_create_file_history(&self) -> *mut c_void {
        unsafe { ffi::wxDocManager_OnCreateFileHistory(self.as_ptr()) }
    }
...
pub trait DocTemplateMethods: ObjectMethods {
    ...
    fn create_view<D: DocumentMethods>(&self, doc: Option<&D>, flags: c_long) -> *mut c_void {
        unsafe {
            ...
            ffi::wxDocTemplate_CreateView(self.as_ptr(), doc, flags)
        }
    }
    ...
    // BLOCKED: fn GetPageSetupDialogData()
    // BLOCKED: fn GetPageSetupDialogData1()
...
pub trait DocumentMethods: EvtHandlerMethods {
    // DTOR: fn ~wxDocument()
    fn add_view(&self, view: *mut c_void) -> bool {
        unsafe { ffi::wxDocument_AddView(self.as_ptr(), view) }
    }
    ...
    fn get_first_view(&self) -> *mut c_void {
        unsafe { ffi::wxDocument_GetFirstView(self.as_ptr()) }
    }
    ...
    // BLOCKED: fn LoadObject()
    // BLOCKED: fn LoadObject1()
    ...
    fn remove_view(&self, view: *mut c_void) -> bool {
        unsafe { ffi::wxDocument_RemoveView(self.as_ptr(), view) }
    }
    fn save(&self) -> bool {
    ...
    // BLOCKED: fn SaveObject()
    // BLOCKED: fn SaveObject1()
    ...
    fn update_all_views<O: ObjectMethods>(&self, sender: *mut c_void, hint: Option<&O>) {
        unsafe {
            let hint = match hint {
                Some(r) => r.as_ptr(),
                None => ptr::null_mut(),
