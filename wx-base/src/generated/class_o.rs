...
wxwidgets! {
    ...
    /// - [`Object`] represents a C++ `wxObject` class instance which your code has ownership, [`ObjectFromCpp`]`<true>` represents one which don't own.
    ...
    #[doc(alias = "Object")]
    class Object
        = ObjectFromCpp<false>(wxObject) impl
        ObjectMethods
}
...
impl Clone for ObjectFromCpp<true> {
    fn clone(&self) -> Self {
        Self(self.0)
    }
}
...
wxwidgets! {
    ...
    /// - [`ObjectRefData`] represents a C++ `wxObjectRefData` class instance which your code has ownership, [`ObjectRefDataFromCpp`]`<true>` represents one which don't own.
    ...
    class ObjectRefData
        = ObjectRefDataFromCpp<false>(wxObjectRefData) impl
...
impl Clone for ObjectRefDataFromCpp<true> {
    fn clone(&self) -> Self {
        Self(self.0)
    }
}
